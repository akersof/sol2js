...
exports.compile = async (fileName, outDir) => {
    ...
    const jsonBaseName = fileName.slice(0, -4) + '.json';
    const jsBaseName = fileName.slice(0, -4) + '.js';
...
`;
    ...
    await fs.writeFile(path.join(absoluteOutDir, fileName.slice(0, -4) + '.js'), jsContent + classContent);
